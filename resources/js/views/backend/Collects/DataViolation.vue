              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_violation"></v-text-field>
  
                    <v-text-field label="Code Genre" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_violation"></v-text-field>
                </v-card-text>
                ...
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr v-for="item in fetchData" :key="item.id">
                      <td>{{ item.nom_violation }}</td>
                      <td>{{ item.code_violation }}</td>
                      <td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        disabled: false,
        edit: false,
        //tperso_division id,nom_violation,code_violation,author
        svData: {
          id: "",
          nom_violation: "",
          code_violation :""
    ...
    methods: {
      ...
      testTitle() {
        if (this.edit == true) {
          ...
          this.titleComponent = "Ajout Genre";
        }
      }
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_data_violation?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          this.isLoading(true);
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_data_violation`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_data_violation/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nom_violation;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_data_violation/${id}`).then(
            ({ data }) => {
