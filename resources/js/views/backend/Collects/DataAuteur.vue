              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_auteur"></v-text-field>
  
                    <v-text-field label="Code Auteur" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_auteur"></v-text-field>
                </v-card-text>
                ...
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr v-for="item in fetchData" :key="item.id">
                      <td>{{ item.nom_auteur }}</td>
                      <td>{{ item.code_auteur }}</td>
                      <td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        disabled: false,
        edit: false,
        //tperso_division id,nom_auteur,code_auteur,author
        svData: {
          id: "",
          nom_auteur: "",
          code_auteur :""
    ...
    methods: {
      ...
      testTitle() {
        if (this.edit == true) {
          ...
          this.titleComponent = "Ajout Auteur";
        }
      }
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_data_auteur?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          this.isLoading(true);
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_data_auteur`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_data_auteur/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nom_auteur;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_data_auteur/${id}`).then(
            ({ data }) => {
