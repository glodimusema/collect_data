              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_ethni"></v-text-field>
  
                    <v-text-field label="Code Ethni" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_ethni"></v-text-field>
                </v-card-text>
                ...
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr v-for="item in fetchData" :key="item.id">
                      <td>{{ item.nom_ethni }}</td>
                      <td>{{ item.code_ethni }}</td>
                      <td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        disabled: false,
        edit: false,
        //tperso_division id,nom_ethni,code_ethni,author
        svData: {
          id: "",
          nom_ethni: "",
          code_ethni :""
    ...
    methods: {
      ...
      testTitle() {
        if (this.edit == true) {
          ...
          this.titleComponent = "Ajout Ethni";
        }
      }
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_data_ethni?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          this.isLoading(true);
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_data_ethni`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_data_ethni/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nom_ethni;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_data_ethni/${id}`).then(
            ({ data }) => {
